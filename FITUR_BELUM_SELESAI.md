...
> *Dokumen ini berisi hasil analisis fitur yang belum selesai, komponen yang masih menggunakan mock data, serta endpoint API yang belum diimplementasikan.*
...
- [ ] **`POST /api/admin/student/add`** : Tambah siswa baru beserta pembuatan akun login (`users` + `siswa`).
...
1. **SQL Error di Backend Service**:
   - [`admin-student-service.js`](file:///c:/Users/Asus/Documents/Perkuliahan/Semester%205/pemrograman%20web/PKM/absensi-sekolah/backend/src/services/admin/admin-student-service.js#L48): Query SQL `findAllClasses` memiliki syntax error:
     ...
     ```
     *(Terdapat koma ekstra sebelum `FROM kelas` yang akan menyebabkan query gagal saat dipanggil)*.
