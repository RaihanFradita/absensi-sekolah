// Konstanta yang dipakai di seluruh aplikasi.
// Menjaga string role/status di satu tempat supaya mudah diubah dan konsisten.

// Nama aplikasi mengikuti nama sekolah. SHORT_NAME dipakai di tempat sempit
// (sidebar mobile, tab browser), FULL_NAME dipakai di halaman Login / footer.
export const APP_NAME = 'SMP Muhammadiyah 4 Cipondoh';
export const APP_SHORT_NAME = 'SMP Muhammadiyah 4';
export const APP_FULL_NAME = 'SMP Muhammadiyah 4 Cipondoh Tangerang';

export const ROLES = {
  STUDENT: 'student',
  TEACHER: 'teacher',
  ADMIN: 'admin',
};

export const ATTENDANCE_STATUS = {
  PRESENT: 'present',
  LATE: 'late',
  ABSENT: 'absent',
  NOT_YET: 'not_yet',
};

export const ATTENDANCE_STATUS_LABEL = {
  [ATTENDANCE_STATUS.PRESENT]: 'Hadir',
  [ATTENDANCE_STATUS.LATE]: 'Terlambat',
  [ATTENDANCE_STATUS.ABSENT]: 'Tidak Hadir',
  [ATTENDANCE_STATUS.NOT_YET]: 'Belum Absen',
};

export const SESSION_STATUS = {
  ACTIVE: 'active',
  ENDED: 'ended',
};

// Key penyimpanan token auth di localStorage. Password TIDAK PERNAH disimpan.
export const AUTH_TOKEN_KEY = 'schoolattend_token';
export const AUTH_USER_KEY = 'schoolattend_user';

// Foto profil disimpan lokal (localStorage) per akun karena belum ada
// endpoint upload foto di backend. Ganti ke pemanggilan API sungguhan
// begitu endpoint-nya tersedia (lihat StudentProfile.jsx).
export const PROFILE_PHOTO_KEY_PREFIX = 'schoolattend_profile_photo_';

export const DEFAULT_PAGE_SIZE = 10;
