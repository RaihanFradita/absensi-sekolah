...
/**
 * rows: [{ id, date, time, status }]
 * Satu baris mewakili satu kehadiran harian, bukan kehadiran per mata pelajaran.
 */
          ...
          <tbody className="divide-y divide-slate-200 dark:divide-slate-800">
            {rows.map((row) => (
              <tr key={row.id} className="bg-white dark:bg-slate-900">
                ...
                <td className="px-4 py-3"><AttendanceStatus status={row.status} /></td>
              </tr>
            ))}
          </tbody>
        </table>
