...
export default function AttendanceHistory() {
  ...
  return (
    <PageContainer title="Riwayat Kehadiran" description="Satu catatan untuk setiap hari sekolah">
      <Card className="mb-6">
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
          <Input label="Tanggal" type="date" value={filters.date} onChange={(e) => updateFilter('date', e.target.value)} />
          <div>
            ...
            >
              ...
              {Object.values(ATTENDANCE_STATUS).filter((s) => s !== ATTENDANCE_STATUS.NOT_YET).map((status) => (
      ...
      {!isLoading && error && (
        <EmptyState title="Gagal memuat riwayat" description={error} action={<Button variant="secondary" icon={RefreshCw} onClick={refetch}>Coba Lagi</Button>} />
      )}
      {!isLoading && !error && data && (
        data.rows?.length > 0 ? (
          <>
            ...
            {totalPages > 1 && (
              <div className="mt-4 flex items-center justify-between">
                <p className="text-sm text-slate-500 dark:text-slate-400">Halaman {page} dari {totalPages}</p>
                <div className="flex gap-2">
                  <Button variant="secondary" size="sm" icon={ChevronLeft} disabled={page <= 1} onClick={() => setPage((p) => Math.max(1, p - 1))}>Sebelumnya</Button>
                  <Button variant="secondary" size="sm" disabled={page >= totalPages} onClick={() => setPage((p) => Math.min(totalPages, p + 1))}>Selanjutnya <ChevronRight className="ml-1.5 h-4 w-4" /></Button>
                </div>
              </div>
            )}
          </>
          ...
          <EmptyState
            ...
            description={hasActiveFilters ? 'Tidak ada data yang cocok dengan filter.' : 'Riwayat akan muncul setelah kamu melakukan absensi harian.'}
          />
        )
      )}
    </PageContainer>
