...
export default function AttendanceReports() {
  ...
  const load = useCallback(async () => {
    ...
    setError("");
    try {
      const d = await attendanceService.getDailyAttendance({ date, className });
      setRows(d.rows || []);
    } catch (e) {
  ...
  return (
    <PageContainer
      title="Data Absensi"
      description="Rekap kehadiran harian sekolah. Perubahan status tidak menghapus data scan asli."
    >
      {loading && <Loading label="Memuat data absensi..." />}
      {!loading && error && (
        <EmptyState
      ...
      {!loading && !error && (
        <DailyAttendanceManager
          ...
          subtitle="Filter berdasarkan tanggal dan kelas, lalu ekspor ke Excel sesuai hak akses."
          rows={rows}
          date={date}
          setDate={setDate}
          classes={classes}
